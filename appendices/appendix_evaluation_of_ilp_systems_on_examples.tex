...
However, this is not learnable since Toplog's mode declarations do not allow to specify \tc{in\_language(s(A))} in a head neither in a body. Toplog outputs:
...
modeb(0,10,pos,anybody("+a")).goal(g).
a(susan; adam; frog).
anybody(susan; adam).
female(susan).male(adam).
