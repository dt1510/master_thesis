...
\begin{itemize}
\item Which ILP systems are capable of learning models who language is regular?
\item Which ILP systems are capable of learning models who language is context-free?
\end{itemize}
...
\begin{defn}
A top theory $\top$ is $\{x \in \mathbb{N} : \psi_\top(x)\}$,
a set definable by $\psi_\top$.
\end{defn}

\begin{defn}
A bottom theory $\bot$ is $\{x \in \mathbb{N} : \psi_\bot(x)\}$,
a set definable by $\psi_\bot$.
\end{defn}

\begin{defn}
A \emph{top theory inductive inference system} TIIS is a computable function
$f: \mathcal{N} \times \mathbb{N} \times \mathbb{N} \to \mathbb{N},
...
\begin{defn}
Given sets $P, N$, a \emph{specialization} of an TIIS $f$ from the top theory $\top$ is a sequence $\{S_i\}$ of sets such that $S_1=\top$,
$S_{i+1}=f(P, N, S_i)$.
\end{defn}
A reader should notice that $S_1 \supseteq S_2 \supseteq S_3 ...$.
