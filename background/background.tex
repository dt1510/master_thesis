...
Like Tal, Xhail returns multiple possible hypotheses for a given learning problem.
\begin{lstlisting}
...
\begin{lstlisting}
1. fries(X1) :- offer(X1),bistro(X1),food(X1).
2. fries(X1) :offer(X1),bistro(X1),food(X1).
3. fries(X1) :- food(X1). 4.fries(X1) :- food(X1).
\end{lstlisting}
where the 3rd and the 4th hypotheses are the same.
