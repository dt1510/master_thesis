...
\begin{defn}
A \emph{language of enquiry} $\mathcal{L}$ is a model-theoretic language given by constant symbols, relation symbols and function symbols. A language of enquiry $\mathcal{L}$ is a superlanguage of two languages: an \emph{observational language} $\mathcal{L}_o$ and a \emph{hypothesis language} $\mathcal{L}_h$. We call the computable restrictions on the language $\mathcal{L}$ to the hypothesis language $\mathcal{L}_h$ to be a \emph{language bias}.
\end{defn}
