...
\subsection{Logics\cite{stanford2010}}
\begin{defn}
...
\subsection{Inductive Inference Setting}
...
A (general) problem of induction is a functional problem whose input is a pentuple $P=\langle L, L_o, L_h, O, B\rangle$ where $L$ is a language of enquiry with its sublanguages $L_o$, $L_h$, observations $O$ and the background knowledge $B$. A solution to a problem of induction (the output) is a theory $A$ meeting the entailment criterion
...
\section{Problem of inductive logic programming}
...
A solution to an ILP problem is a set of sentences $H \in \mathcal{H}$ called a hypothesis satisflying the conditions
...
\subsection{ILP systems, frameworks, methods}
...
TODO copy the existing material with references

\subsection{ILP methods}

\subsubsection{Inverse Resolution}

\subsubsection{Resolution}

\subsubsection{Inverse Entailment}

\subsection{ILP systems and frameworks}
...
\subsubsection{CF-induction method}

\subsubsection{Least Common Generalization method}

\subsubsection{Least General Generalization method}

\subsubsection{Induction on Failure framework}
...
\subsubsection{Confucius}

\subsubsection{Quinlan}
...
\subsection{Meta-Interpretive Learning framework}
...
\subsection{Correct ILP system}
An ILP system is correct if it solves an ILP problem, i.e. its output is a solution to an ILP problem.

\chapter{Hypotheses space bias}

TODO: make abstractions of the systems and frameworks so that it would be easy to reason about their biases.
