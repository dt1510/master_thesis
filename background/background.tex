...
find a computable set $S = \{n \in \mathbb{N} : \mathbb{N} \models \phi(n) \} \subseteq \mathbb{N}$ that has a defining formula $\phi$ of the least
...
\chapter{Language of reality learnability}
The reality $\mathcal{M}$ that we would like an ILP system to learn can be mapped to a countably infinite set a language $L(\mathcal{A})$ generated by an alphabet $\mathcal{A}$,
$\exists \Phi: \mathcal{M}_H \to L(\mathcal{A})$ where $\mathcal{M}_H$ is a Herbrand model of $\mathcal{M}$. We are interested in comparing ILP systems based on their capability to learn a language of the reality $Im(\Phi(\mathcal{M}_H)$.
\begin{itemize}
\item Which ILP systems are capable of learning models whose language is regular?
\item Which ILP systems are capable of learning models whose language is context-free?
\end{itemize}

We introduce the following abstraction of an ILP system in a computability systems.
\begin{defn}
An \emph{inductive inference system} IIS is a computable function
$f: \powerset{\mathbb{N}} \times \powerset{\mathbb{N}} \to \powerset{\mathbb{N}},
\langle P, N \rangle \mapsto S$
such that $P \subseteq \mathbb{N}, N \cap S = \emptyset$
and $S$ is a computable set.
\end{defn}

\section{Bottom and top theories in computability setting}
...
\begin{defn}
A \emph{top theory inductive inference system} TIIS is a computable function
$f: \powerset{\mathbb{N}} \times \powerset{\mathbb{N}} \times \powerset{\mathbb{N}} \to \powerset{\mathbb{N}},
\langle P, N, \top \rangle \mapsto S$
such that $P \subseteq \mathbb{N}, N \cap S = \emptyset, P \subseteq \top$.
\end{defn}

\begin{defn}
A \emph{bottom theory inductive inference system} BIIS is a computable function
$f: \powerset{\mathbb{N}} \times \powerset{\mathbb{N}} \times \powerset{\mathbb{N}} \to \powerset{\mathbb{N}},
\langle P, N, \bot \rangle \mapsto S$
such that $P \subseteq \mathbb{N}, N \cap S = \emptyset, \bot \subseteq S$.
\end{defn}

Notice that when we fix a 3rd argument, both TIIS and BIIS are IISs.

\begin{defn}
Given sets $P, N$, a \emph{specialization} of an TIIS $f$ from the top theory $\top$ is a sequence $\{\top_i\}$ of sets such that $\top_1=\top$,
$\top_{i+1}=f(P, N, \top_i)$.
\end{defn}
A reader should notice that $\top_1 \supseteq \top_2 \supseteq \top_3 ...$.
The sets of a specialization sequence are formed by adding a conjuct $\phi_{i+1}$ to the defining formula of $\top_{i+1}$,
i.e. $\top_{i+1}=\{x \in \mathbb{N} : \psi_i(x) \wedge \phi_{i+1}(x) \}$.
