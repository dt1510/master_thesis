...
\section{Logics\cite{stanford2010non-monotonic}}
\begin{defn}
...
\section{Open problems in ILP}
\begin{itemize}
\item Definition of an ILP problem: there are many variations of an ILP problem setting, which one should we use and why? Do these ILP problem settings realy address the problem that we are trying to solve?
...
\chapter{Designing a custom ILP system}
After having reviewed ILP systems, we aim to design a custom ILP system that would address some of the problems we have encountered in the ILP systems.

\section{Suggested solutions}
   
\subsection{Inverse Subsumption for Complete Explanatory Induction}\cite{yamamoto2012inverse}
Our suggested solutions are an extension of the work by Yamamoto, et al. in which they provide a complete theoretical framework for solving the problem of an explanatory induction.
...
$\tau'(S)$ denotes the clausal theory obtained by removing every clause from $S$ that factors to a tautology. $I_H$ is an induction field of not necessarily ground literals.
...
$H$ subsumes $\tau(M(F \cup Taut(I_H)))$,
$H$ does not subsume $\tau(M(F- \cup Taut(I_H)))$.
\end{itemize}
\end{defn}
\section{Comparison of Upward and Downward Generalizations in CF-Induction}\cite{yamamoto2012comparison}
\subsection{Faster consistency check}
...
\subsection{Bidirectional hypothesis search using upward and downward generalization}
Searching a hypothesis by applying both upward and downward generalization may be more efficient than applying one generalization method only. This intuition comes from the path search. Imagine two points A, B at the distance $2r$ in a plane. Searching from A (or B), we need to explore the area in a radius $2r$ until we find B (or A), the total area to be explored by this monodirectional search is $4 \pi r^2$. However, when searching from A and B mutually the closest point of the intersection is in a half way between A and B. Therefore the total area to be explored in a 2d plane by a bidirectional search from A and B is $2 \pi r^2$ which is twice less as for the monodirectional search.

In general a bidirectional search in a $n+1$-dimensional plane is $2^n$ times more efficient as the monodirectional search in the same plane.

The problem statement is to estimate the theoretical maximum efficiency (in terms of the time complexity relative to the two monodirectional methods of upward generalization and downward generalization respectively) for finding the target hypothesis by using both upward and downward generalization method and provide the corresponding heuristics algorithm.
...
\subsection{IE algorithm\cite{yamamoto2011}}
...
\section{Completeness in generalization\cite{yamamoto2011}}
\begin{center}
