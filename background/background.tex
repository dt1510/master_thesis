...
Some ILP systems like Aleph in additions to mode declarations use determination statements\cite{aleph2007} to impose a finer control on a search space.
...
\begin{defn}
Let $\mathcal{D}=\langle d_1(tn_1/ta_1, bn_1/ba_1), ..., d_n(tn_n/ta_n, bn_n/ba_n)\ \rangle$ be an ordered list of determinations. A formula $H=h \leftObjectImplies b_1, ..., b_m$ is in a a \emph{determination language} $\mathcal{L}(\mathcal{D})$ iff
1) $\mathcal{D}\not=\emptyset$, 2) $tn_1=h$, 3) $\forall b_i (1 \ge i \ge m) \exists j (1 \ge j \ge n) bn_j=b_i$.
\end{defn}

\begin{exmp}
Let $\mathcal{D}$ be the list of the determinations
\begin{lstlisting}
...
Then for the hypothesis bias (determination language) $\mathcal{H}=\mathcal{L}(\mathcal{D})$,
...
(\emph{the characteristic literals} of $\mathcal{P}$) is a subset of literals in a language $L$ and is closed under instantlatlon (that is, if a literal containing variables is in $Lit$, then all its instances are also in $Lit$), and $Cond$ is a certain condition to be satisfied. 
A clause $C$ is said to belong to a producnon field $\mathcal{P} = \langle Lit, Cond \rangle$ if every literal in $C$ belongs to $Lit$ and $C$ satisfies $Cond$.
\end{defn}
