...
\tc{:-modeh(*, cook(+person)).} was defined after \tc{:-modeh(*, woman(+person)).}.
...
\end{lstlisting}
If our target hypothesis had only one function symbol in its predicate, then a simpler hypothesis \tc{in_language(s(A)) :- in_language(A).} could indeed be learnt.

\subsection{Other assumed limitations}
Any hypothesis learnt in Aleph has to be a Horn theory. Aleph cannot learn second-order logic statements.
