...
\begin{defn}
A bias $P$ of the language $L$ is \emph{sufficiently weak} with respect to the theory $T$ iff $P$ is defined on $L$ and there exists an axiomatization $A$ of the theory $T$ such that $\forall \phi. \phi \in T \implies P(\phi)$. If $P$ is not sufficiently weak with respect to the theory $T$ we say that $P$ is \emph{too strong} with respect ot the theory $T$.
\end{defn}

\begin{defn}
Let $P_1$, $P_2$ be the biases on the language $L$. $P_1$ is \emph{weaker} than $P_2$ and $P_2$ is \emph{stronger} than $P_1$ denoting $P_1 \le P_2$ iff
$\forall x \in L. P_2(x) \implies P_1(x)$.
\end{defn}

\begin{exmp}
Define the biases $P_k$ for $k \in \mathbb{Z}_{\ge 0}$ by $P_k(\phi)$ iff $\phi$ consists of at most $k$ distinct characters. Let $T$ be the theory of the elementary class of the partial orders. Let $A=\{\phi_1, \phi_2, \phi_3\}$ where
...
$P_0 \le P_1 \le P_2 \le P_3 \le ... \le P_8 \le ...$. Thus $P_3$ is stronger than $P_8$.
\end{exmp}

\subsection{The problem of the correct bias}
