...
Define an equivalence relation over the hypotheses space $\mathcal{H}$: $\Sigma \sim \Gamma \iff s(\Sigma)=s(\Gamma)$ where $s$ is the \emph{score function} $s:\mathcal{H} \to \mathbb{R}$. Find an axiomatization $A$ of the theory $B \union O$ with the greatest score $s$. The solution to a problem of induction (the output) is $A$ called \emph{an induced theory} from the background knowledge $B$ and the observations $O$.
