...
in which a predicate $woman$ is a head symbol although it was not specified in the mode declaration. Once it is specified in a declaration as
\begin{lstlisting}    
head_modes([
   %woman(+person)
]).
...
\subsection{Unlearnability of headless clases and negative examples}
In cases when a hypothesis of the clausal form with no head could explain the examples, Imparo cannot learn such a clause.
...
However, the reader may observe that the observations of the $woman$ predicate together with the background knowledge imply that all observed males are men, thus a hypothesis
\begin{lstlisting}
man(X) :- male(X).
\end{lstlisting}
could be learnt which would be more general than its instances, but it is not learnt.
...
\subsection{Learnability of the Kleene star}
...
Imparo can also learn a rule applying the Kleene star operator on a nested term structure.
...
However, a more general a shorter hypothesis would be
...
The same fact applies even if we move part of observations to the background knowledge:
...
and expect to learn a general hypothesis

\begin{lstlisting}
even(A) :- even(s(s(A)).
\end{lstlisting}

but only learn the examples
\begin{lstlisting}
even(0):-true
even(s(s(s(s(s(s(0))))))):-true
even(s(s(0))):-true
