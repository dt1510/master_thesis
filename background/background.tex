...
This limitation can be considered as a bias - the system searches how to explain the observations but it does not use the information from these observations to do so. If we added the observation
\tc{woman(A):-female(A)} to the background knowledge and run the program again, then Imparo would learn the hypothesis \tc{man(X) :- male(X).}.
...
Systems like Prolog check for inconsistencies before learning is started. However, in some definitions of an ILP problem, an assumption that the background knowledge is consistent with the observations is made.
