...
$A \union B \Vvdash O$ where the entailment relation and other criteria are specified for a particular version of a problem.
...
Define an equivalence relation over the hypotheses space $\mathcal{H}$: $\Sigma \sim \Gamma \iff s(\Sigma)=s(\Gamma)$ where $s$ is the \emph{score function} $s:\mathcal{H} \to \mathbb{R}$. Find an axiomatization $A$ of the theory $O$ with the greatest score $s$.
