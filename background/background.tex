...
woman(tom).
\end{lstlisting}
does not return any hypothesis, even we specify the hypothesis in the mode declaration by \tc{:- woman(+person)}.
...
woman(ann).
\end{lstlisting}
returns a hypothesis \tc{woman(ann).} which is inconsistent with the negative examples.
...
Aleph prefers learning a hypothesis whose determination declaration has been defined later.
...
man(A) :- male(A).
\end{lstlisting}
In both cases, the induced hypothesis had a head atom declared in an earlier determination. This is an opposite bias to the Imparo's preference over the later mode declarations.

\subsection{Inexplicability by predicates of greater arity}
Aleph cannot explain the examples from the background knowledge with the predicates of arity greater than 2.
...
returns back the positive examples. However, a more general hypothesis could explain the examples
\begin{lstlisting}
english(A) :- english_couple(A,B).
english(B) :- english_couple(A,B).
\end{lstlisting}

This restricts the hypotheses space to the clauses with monadic bodies. However, learning of hypotheses polyadic predicate symbols in their head is still possible.
