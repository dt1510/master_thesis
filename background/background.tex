...
Imparo has examples in its default bias without the need to declare them in the mode declarations.
...
Imparo is capable to learn multiple hypotheses at one time.
...
\subsection{Limited use of observations}
Imparo cannot make deductions from the observations and the background knowledge to induce a general hypothesis.
...
learns the following hypothesis:
...
However, the reader may observe that the observations of the \tc{woman} predicate together with the background knowledge imply that all observed males are men, thus a hypothesis \tc{man(X) :- male(X).} could be learnt which would be more general than its instances, but it is not learnt.

This limitation cannot also be considered as a form of a bias - the system searches how to explain the observations but it does the information from these observations to do so. If we added the observation
\tc{woman{A}:-female(A)} to the background knowledge and run the program again, then Imparo would learn the hypothesis \tc{man(X) :- male(X).} too.
...
In this regard, Imparo treats function symbols as a syntactic sugar - a predicate with a function symbol is treated as a predicate symbol, once could simply replace \tc{woman(sister(x))} by \tc{woman\_sister(x)} in examples and mode declarations and add the following to the background knowledge:
\begin{lstlisting}
...
Imparo is capable of learning a language constructed from the alphabetical symbols and a Kleene star operation.
...
To the authors knowledge, other ILP systems do not deal with the case of non-literal observations, however the works by Plotkin dealt with the generalization of a general set of clauses.
...
Systems like Prolog check for inconsistencies before the learning is started. However, in some definitions of an ILP problem, an assumption that the background knowledge is consistent with the observations is made.
...
This search bias is stronger than a criterion of favouring a stronger or a weaker hypothesis. By adding to the background knowledge
\begin{lstlisting}
policeman(X) :- male(X).
\end{lstlisting}
or its converse
