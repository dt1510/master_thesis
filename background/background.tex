...
\begin{defn}
\emph{A hypothesis space} or a search space is the set $\mathcal{H}$ of all consistent subsets of a hypothesis language $L_h$. A formula $H \in \mathcal{H}$ is called \emph{a hypothesis}.
\end{defn}
...
TODO: more precise.
\item Solo-generalization bias. The hypothesis space is restricted to the generalization of a single example. Reference: Muggleton etal, MC-Toplog.
...
\item Score function bias. Having two correct hypotheses $H_1$, $H_2$ such that $B \union H_1 \models E$ and $B \union H_2 \models E$ choose the hypothesis with the greater score by the function $s:$Search space$ \to \mathbb{R}$.
\end{itemize}
\subsection{Progol}
\begin{itemize}
...
By inducing the $\Pi_1$ statement into the theory, it may be impossible for some true $\Pi_2$ statements to be part of the theory.
