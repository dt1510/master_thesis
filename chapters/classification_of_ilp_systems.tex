...
One may define a hypothesis bias $\mathcal{H}$ by enumerating all the hypotheses $H \in \mathcal{H}$. If $\#\mathcal{H}$ is large, then it may be impractical. ILP systems specify constrains on a hypothesis space with alternative constructions. A mode declaration\cite{muggleton1995inverse} specifies the predicates allowed in a hypothesis.

\begin{defn}
A \emph{mode declaration} for an ILP task $I \mapsto O$.
\end{defn}
...
\subsection{Approximating requirements}
Toplog - hypothesis.
