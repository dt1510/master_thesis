...
\begin{defn}
Let $R \subseteq L$ be a set of rules, let $S$ be any mathematical object with a syntactic denotation $D_S$. Then $r \in R$ \emph{constraints} $S$ iff $r$ is formed from a function or a predicate symbol in $D_S$.
\end{defn}
...
A rule $r \in R_2 \setminus R_1$ is an \emph{extending rule of an extending component} $c$ iff the formula $r$ contains any of the symbols of $c$.
\end{defn}
