...
\paragraph{Toplog}
Toplog's modeb declarations are compatible with Progol, specification of the recall is optional, however, for modeh declarations recall cannot be specified\cite{santos2008toplogWebsite}.
\begin{exmp}
\begin{lstlisting}
...
\subsubsection{Modet declarations}
In addition to \tc{modeb}, \tc{modeh} mode declarations, Toplog uses \tc{modet} declarations. Every \tc{modet} declaration can be transformed to a \tc{modeb} declaration. For example \tc{modet(l2ht(+list/[H|T], -any/H, -list/T))} has its correspondent \tc{modeb} declaration \tc{modeb(+list=[-any|-list])}.
...
\captionof{table}{Classification by mode declaration bias} \label{tab:title} 
 \begin{tabular}{| l | l | l | l | l | l | l |}
    \hline
    ILP system & Progol & Aleph & Toplog & Xhail & Imparo & Tal \\ \hline
    Mode declaration bias &  yes & yes & yes & yes & yes & yes \\ \hline    
    Prolog compatible & yes & yes & no &  \\ \hline
    Recall modeb support & yes & yes & yes & \\
    Recall modeh support & yes & yes & no & \\ \hline
    \hline
  \end{tabular}
\end{minipage}
