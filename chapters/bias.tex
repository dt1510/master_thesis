...
\begin{proposition}
Let $Cond_1, Cond_2$ be meta-constraint on a language $L$.
Then the following are meta-constraints on the language $L$:
...
\begin{remark}
ILP systems we classify allow only a specification of a conjunction of meta-constraints. However, from the theoretical point of view, their negations and disjunctions make sense and it is possible to design an ILP system with an extended meta-constraint specification.
\end{remark}
...
A production field is a generalization of mode declarations, determinations and meta-constraints.
...
Given production fields defining the bias of mode declarations and determinations, productions fields with additional meta-constraints can be constructed.
...
The objective of this subsection is to classify ILP systems based on their peculiarities related to the language bias: mode declarations, determinations and meta-constraints.
...
meta-constraints supplement mode declarations and determinations in specifying the language bias. Several meta-constraints are selected and their support is evaluated across the ILP systems classified:
...
\end{lstlisting}
Their change did not affect the properties examined, hence the author assumes Progol does not support any of the meta-constraints.
...
\paragraph{Summary}
The support for the meta-constraints of interest is summarised.
...
If one considered the chosen meta-constraints as representative, then it could be deduced that Progol and Xhail have the most trivial system of specifying the language bias with meta-constraints whereas Aleph and Imparo have the most powerful system.
