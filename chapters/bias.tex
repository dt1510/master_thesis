...
\begin{itemize}
\item construct the \emph{top theory} $\top$ (a bias represented as a logic theory specified on the input by the user),
...
\paragraph{Top theory\cite{muggleton2008toplog}}
In general, a \emph{top theory} is any logic theory, it makes sense to reason about such a logic theory wrt some hypothesis.
\begin{defn}
...
\begin{exmp}\cite{muggleton2008toplog}
From the mode declarations
\begin{lstlisting}
modeh(mammal(+animal)).
modeb(has milk(+animal)).
modeb(has eggs(+animal)).
\end{lstlisting}
a correspondent constructed top theory is a set of clauses
\begin{lstlisting}
mammal(X) :- $body(X).
: $body(X) :- .%emptybody
: $body(X) :- has milk(X),$body(X).
: $body(X) :- has eggs(X),$body(X).
\end{lstlisting}
\end{exmp}
