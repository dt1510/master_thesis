...
We next define the target hypotheses using the notion of an induction field $\mathcal{I}_H$ (definition \ref{induction_field_definition}), together with a bridge theory $F$ as follows:
...
\subsection{Negative examples}\label{extension_negative_examples}
...
\emph{Completeness and soundness of extension to negative examples}.
...
\begin{algorithm}\label{negative_examples_antisubsumption_algorithm}
\emph{Checking consistency of hypothesis with negative examples by inverse subsumption}
\begin{itemize}
\item Given $B$, $H$, $E^-$,
\item Construct $T=B \cup Taut(\mathcal{I}_H)$,
\item Step 3: If $E^-$ does not have any negative examples left, then return \tc{consistent},
\item Take a negative example $e$ from $E^-$:
\item Construct $S=\tau(M(\{\neg e\} \cup T))$,
\item if $H$ can be derived by anti-subsumption from $S$,
then return \tc{inconsistent} otherwise go to step 3.
\end{itemize}
\end{algorithm}

\subsection{Relaxation of minimal complements to a maximal bridge theory}
\label{relaxation_minimal_complements_algorithm}
...
Based on the work by Yamamoto et al. on the generalization operators\cite{yamamoto2008towards} we prove that every theory $S$ subsuming a theory $T$ can be derived by anti-subsumption using two generalization operators dropping and anti-instantiation.
...
\begin{proposition}\label{inverse_subsumption_operators_completeness}
\emph{(Completeness of dropping and anti-instantiation for anti-subsumption)}.
...
The use of the anti-entailment\cite{yamamoto2012inverse} in the computation of a hypothesis is an expensive operation as compared to the inverse subsumption. The use of the anti-entailment instead of the anti-subsumption may not preserve the completeness. However, we solve an open problem from \cite{yamamoto2012inverse} that for every correct hypothesis $H$ wrt $B, E^{+}, E^{-}$ there is a connected theory $T$ wrt $B, E^{+}, E^{-}$ subsumed by it.
...
\end{thm}
The theorem \ref{yamamoto2012inverseTheorem4} is proved in \cite{yamamoto2012inverse}. We prove:
...
\end{thm}
The theorem \ref{implicationByGroundClauses} is proved in \cite{nienhuys1997foundations}.
...
The most important result of this thesis is:
...
We conjecture a direct correspondence between the theory subsuming a target hypothesis constructed by minimal complements and the connected theory.
