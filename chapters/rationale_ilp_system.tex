...
Rationale solves the defined ILP task:
\begin{defn}
An ILP task $f:I \mapsto O$ of \emph{explanatory induction with negative examples with a classical consequence operator} is defined by $\langle f, dom(f), cod(f), R \rangle$ where
...
\subsection{Prune the hypotheses by the negative examples}
